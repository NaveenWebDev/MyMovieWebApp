    ...
    <div className="lg:w-1/2 md:w-2/3 mx-auto">
      <div className="flex flex-wrap -m-2">
        ...
        <div className="p-2 w-full">
          <div className="relative">
            <label htmlFor="message" className="leading-7 text-sm text-white">Image URL</label>
            <input id="message" name="message" className="w-full rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-2 px-3 resize-none leading-6 transition-colors duration-200 ease-in-out" defaultValue={""} value={form.image} onChange={(e)=>setform({...form, image:e.target.value})} />
          </div>
        </div>
        ...
        <div className="p-2 w-full">
          ...
          <button onClick={addMovie} className="flex mx-auto text-white bg-blue-600 border-0 py-2 px-8 focus:outline-none hover:bg-blue-500 rounded text-lg"> {loading ? <TailSpin height={25} color="white" /> : "submit" } </button>
